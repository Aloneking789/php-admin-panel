...
if(isset($_POST['saveuser'])){
    ...
    redirect('users-create.php', 'Invalid request');
}
...
if(isset($_POST['updateuser'])){
    ...
    $user = getById('users',$userId);
    if($user['status'] != 200){
      redirect('user-edit.php?id','User/Admin created Successfully');
    }

    // Check if any of the required fields are empty
    if($name != '' && $email != '' && $phone != '' && $password != ''){
        // Prepare the SQL query
        $query = "UPDATE users SET name='$name', phone='$phone', email='$email', password='$password', role='$role' WHERE id='$userId'";
